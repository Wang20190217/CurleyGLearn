...
本文，我们将演示“单表”的使用，基于代码生成器，在 `curleyg-module-system` 模块中，开发一个【**用户组**】的功能。
...
![建表语句](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271515981.png)
...
![基于 DB 导入](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271515037.png)
...
![基本信息](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271516972.png)

![字段信息](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271516000.png)
...
- 将【上级菜单】设置为【系统管理】。
- 将【前端类型】设置为“前端项目”对应的“前端类型”。例如说，我们这里演示的是 `curleyg-web-vue3` 前端项目，则选择了【Vue3 Element Plus 标准模版】。

| 前端项目         | 前端类型                   |
| ---------------- | -------------------------- |
| curleyg-web-vue2 | Vue2 Element UI 标准模版   |
| curleyg-web-vue3 | Vue3 Element Plus 标准模版 |
| curleyg-web      | layui 模版                 |
...
![代码预览](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271517295.png)

### 2.4 生成代码

点击 `system_group` 所在行的 [生成代码] 按钮，下载生成代码的压缩包，双击进行解压。

![生成代码](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271517702.png)
...
![复制后端](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271517812.png)

② 将 `ErrorCodeConstants.java_手动操作` 文件的错误码，复制到该模块 ErrorCodeConstants 类中，并设置对应的错误码编号，之后进行删除。操作如下图所示：

![添加错误码](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271517021.png)
...
如果想要开启，可以把 `curleyg.codegen.unit-test-enable` 配置项设置为 `true`，然后重新生成代码。
...
![复制 h2 SQL](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271519160.png)
...
![初始化菜单](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271519764.png)

⑤ Debug 运行 CurleygServerApplication 类，启动后端项目。通过 IDEA 的 [Actuator -> Mappings] 菜单，可以看到代码生成的 GroupController 的 RESTful API 接口已经生效。

![RESTful API 接口](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271519699.png)

### 3.2 前端运行

① 将生成的前端代码，复制到项目中。操作如下图所示：

![复制前端](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271520310.png)

② **重新**执行 `npm run dev` 命令，启动前端项目。点击 [系统管理 -> 菜单管理] 菜单，点击【刷新菜单缓存】，因为前端项目会缓存菜单在内存中的，所以需要刷新一下。

③ 点击 [系统管理 -> 用户组管理] 菜单，就可以看到用户组的 UI 界面。

![前端界面](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271520815.png)
