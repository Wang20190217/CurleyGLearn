...
例如说，在服务上部署了一个 [`curleyg` ]系统，可以支持多个不同的公司使用。这里的**一个公司就是一个租户**，每个用户必然属于某个租户。因此，用户也只能看见自己租户下面的内容，其它租户的内容对他是不可见的。
...
![DATASOURCE 模式](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270932981.png)
...
![SCHEMA 模式](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270932386.png)
...
![COLUMN 模式](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270933782.png)
...
![模式选择](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270934939.png)
...
| 配置项                  | 说明     | 配置文件                                                     |
| ----------------------- | -------- | ------------------------------------------------------------ |
| `culryg.server.tenant`  | 后端开关 | ![示例](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270935308.png) |
| `VUE_APP_TENANT_ENABLE` | 前端开关 | ![示例](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412270935981.png) |
...
| 业务功能 | 说明                                               | 界面                                                         | 代码                                                         |
| -------- | -------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 租户管理 | 配置系统租户，创建对应的租户管理员                 | ![租户管理](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420308.png) | [后端 ](https://github.com/YunaiV/ruoyi-vue-pro/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/service/tenant/TenantServiceImpl.java)[前端](https://github.com/yudaocode/yudao-ui-admin-vue2/blob/master/src/views/system/tenant/index.vue) |
| 租户套餐 | 配置租户套餐，自定每个租户的菜单、操作、按钮的权限 | ![租户套餐](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420681.png) | [后端 ](https://github.com/YunaiV/ruoyi-vue-pro/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/service/tenant/TenantPackageServiceImpl.java)[前端](https://github.com/yudaocode/yudao-ui-admin-vue2/blob/master/src/views/system/tenantPackage/index.vue) |

**下面，我们来新增一个租户，它使用 COLUMN 模式。**

① 点击 [租户管理] 菜单，点击 [新增] 按钮，填写租户的信息。

![新增租户](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420335.png)

② 点击 [确认] 按钮，完成租户的创建，它会自动创建对应的租户管理员、角色等信息。

![租户的管理员、角色](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420972.png)

③ 退出系统，登录刚创建的租户。

![登录界面](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420149.png)
...
技术组件 [`curleyg-spring-boot-starter-biz-tenant` ]，实现透明化的多租户能力，针对 Web、Security、DB、Redis、AOP、Job、MQ、Async 等多个层面进行封装。

### 5.1 租户上下文

[TenantContextHolder ]是租户上下文，通过 ThreadLocal 实现租户编号的共享与传递。通过调用 TenantContextHolder 的 `#getTenantId()` **静态**方法，获得当前的租户编号。绝绝绝大多数情况下，并不需要。
...
![请求示例](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271420988.png)

如果不带该请求头，会报“租户的请求未传递，请进行排查”错误提示。

😜 通过 `curleyg.tenant.ignore-urls` 配置项，可以设置哪些 URL 无需带该请求头。例如说：

![ 配置项](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271456404.png)
...
![ 配置项](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271456812.png)
...
注意！！！默认配置下，Spring Cache 都开启 Redis Key 的多租户隔离。如果不需要，可以将 Key 添加到 `curleyg.tenant.ignore-caches` 配置项中。如下图所示：

![ 配置项](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271456561.png)
...
![TenantUtils 模拟租户](https://curleyg-1311489005.cos.ap-shanghai.myqcloud.com/202412271456777.png)
